...
class MCServer(ABC):
    ...
    def __init__(self, host: str, port: int | None = None, timeout: float = 2):
        if port is None:
    ...
    @classmethod
    def lookup(cls, address: str, timeout: float = 2) -> Self:
...
class JavaServer(MCServer):
    ...
    @classmethod
    async def async_lookup(cls, address: str, timeout: float = 2) -> Self:
